...
Note for multi-module projects: If your project has multiple modules, Jenkins must be setup so that lint only runs on the TOP-MOST module in order to avoid duplicate/false lint warnings. Running lint on the top-most module will run lint on all of the lower modules as well. For instance, Jenkins can be configured so that `analyze${buildVariant}` can be run on the top level module and it will run lint on all lower modules (and compile the warnings), but you must separately run pmd and findBugs on all of the lower modules. 
